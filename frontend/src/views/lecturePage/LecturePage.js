...
export default function LecturePage({ lectureHeader, lectureDesc }) {
  ...
  const [hasAttachedFile, setHasAttachedFile] = useState(true);
  const [attachedFile, setAttachedFile] = useState(null);

  useEffect(() => {
    const loadLecture = async () => {
      const result = await NasmeenAPI.readLecture(param.lectureID);
      setData(result.lecture);
      if (result.lecture.file) {
        const file = await FileAPI.getURL(result.lecture.file);
  ...
  return (
    <div>
      ...
      <div className="mt-[60px] bg-[#F6F5F4] py-[30px] px-[20%] flex flex-col">
        <ReturnButton />
        {data === null || (hasAttachedFile && attachedFile === null) ? (
          <TailSpin
            height="80"
            width="80"
            color="#2B788B"
          ...
          <div className="py-[30px] px-[5%] bg-white flex flex-col w-full rounded-lg">
            ...
            {hasAttachedFile ? (
              <ol>
                <File fileName="Attached File" filePath={attachedFile} />
              </ol>
            ) : (
              <></>
            )}
